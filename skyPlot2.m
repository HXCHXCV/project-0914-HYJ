...
%% Prepare axis ===========================================================
hAxis = newplot(hAxis);
% hAxis = figure('Name','Measured Data','NumberTitle','off');
...
for i = 1:max(size(th))
    ...
    text(rt*snt(i), rt*cst(i), int2str(i*30), ...
        'horizontalalignment', 'center', 'handlevisibility', 'off','FontSize',a);
    ...
    text(-rt*snt(i), -rt*cst(i), loc, ...
        'handlevisibility', 'off', 'horizontalalignment', 'center','FontSize',a);
end
...
for elevation = 0 : 15 : 90
    ...
    % 新代码：线性映射（等间隔）：90°→0，60°→30，30°→60，0°→90
%     elevationSpherical = 90 - elevation;  % 核心修改：线性映射
    
    line(yunit * elevationSpherical, xunit * elevationSpherical, ...
        'lineStyle', ':', 'color', tc, 'linewidth', 0.8, ...
        'handlevisibility', 'off');
    if elevation ~= 0
    text(0, elevationSpherical, num2str(elevation), ...
        'BackgroundColor', 'none', 'horizontalalignment','center', ...
        'handlevisibility', 'off','FontSize',a);
%     set(gca,'FontSize',12);
    end
end
...
% 新代码：线性映射（等间隔）
elSpherical = 90 - el;  % 核心修改：与网格线保持一致的映射


%--- Transform data to Cartesian coordinates ------------------------------
yy = elSpherical .* cos(az * pi/180);  % 后续坐标转换不变
xx = elSpherical .* sin(az * pi/180);
...
%     hpol = plot(hAxis, xx', yy',  'Color', 'r', 'Marker', 'o', 'MarkerFaceColor', 'r','MarkerEdgeColor', 'r', 'MarkerSize', 8, 'LineStyle', 'none');     
    hpol = plot(hAxis, xx(1:4)', yy(1:4)','Marker', 'o','MarkerEdgeColor', [0.78,0.11,0.11], 'MarkerSize', 8, 'LineStyle', 'none','LineWidth',2.15);    
%     hpol = plot(hAxis, xx(1:4)', yy(1:4)','Marker', 'o','MarkerEdgeColor', 'r', 'MarkerSize', 8, 'LineStyle', 'none','LineWidth',2.15); 
%     hold on;
    hpo2 = plot(hAxis, xx(5:10)', yy(5:10)','Marker', 'o','MarkerEdgeColor', 'k', 'MarkerSize', 8, 'LineStyle', 'none','LineWidth',2.15);    
%     hold off
else
...
% plot(hAxis, xx(:,end)', yy(:,end)', 'o','Color', 'r', 'MarkerSize', 12);

%--- Place satellite PRN numbers at the latest position -------------------
for i = 1:length(prn)
    if(prn(i) ~= 0)
        if i >= 1 && i <= 4
            text_color =  [0.78,0.11,0.11];  % 红色
        else
            text_color = 'k';  % 黑色（默认覆盖5-8及可能的更长范围）
        end
        text(xx(i, end)-5, yy(i, end)+3.5, ['  ', int2str(prn(i))], 'color', text_color, 'FontSize', a, 'FontWeight', 'bold','HorizontalAlignment', 'right'); %
%           text(xx(i, end)-5, yy(i, end), ['  ', int2str(prn(i))], 'color', 'k','FontSize',a,'FontWeight', 'bold','HorizontalAlignment', 'right'); %
    end
end
...
axis(hAxis, 'off');
% title ('Sky plot of all visible satellites');
