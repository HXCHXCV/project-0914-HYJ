function [TxTime,satClkErr] = GetTravelTime_deltaT(RxTime,RxPos,eph,settings,RxPos1,delay)
...
for index = 1:length(RxTime)
    ...
    for iterCnt = 1:10   % the max iteration counts
        ...
        traveltimeTemp = norm(satPos - RxPosTemp) / settings.c + norm(RxPosTemp - RxPosTemp1) / settings.c  + delay / settings.c ;
        ...
        traveltimeTemp = norm(Rot_X - RxPosTemp) / settings.c + norm(RxPosTemp - RxPosTemp1) / settings.c  + delay / settings.c;
