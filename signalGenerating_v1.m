function signalGenerating_v1(settings)
...
state = [1,2,3]; %状态选择，真实轨迹采用1和2，对应静止和运动，      欺骗采用3-运动[未用上]

%真实轨迹
s = 3*settings.SpoofingStart_Position/1000;
StartPotion_au_XYZ = settings.StartPotion_au_XYZ ;
[ECEFx,ECEFy,ECEFz,lat,lon,alt] = trajectorySimulation_XYZ(settings, trajectoryTime,state(2),StartPotion_au_XYZ,s);  %接收机运动
% [ECEFx,ECEFy,ECEFz,lat,lon,alt] = trajectorySimulation_XYZ(settings, trajectoryTime,state(1),StartPotion_au_XYZ,0);  %接收机静止
...
%        Delay(1:4) = Delay(1:4) + 0; 
%        Delay(1:4) = Delay(1:4) + 100; 
%        Delay(1:4) = Delay(1:4) + 150; 
%        Delay(1:4) = Delay(1:4) + 300; 
